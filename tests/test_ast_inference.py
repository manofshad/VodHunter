...
class FakeModelOutput:
    ...
    def __init__(self, batch_size: int):
        self.pooler_output = FakePoolerOutput(np.ones((batch_size, 4), dtype=np.float32))
...
class TestAstInference:

    def test_compute_embeddings_splits_audio_into_one_second_chunks(self) -> None:
        audio = np.ones(32000, dtype=np.float32)
        embeddings, timestamps = compute_ast_embeddings(audio_data=audio, sample_rate=16000, feature_extractor=FakeFeatureExtractor(), model=FakeModel(), device='cpu', offset_seconds=1.5)
