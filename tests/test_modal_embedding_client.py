...
class TestModalEmbeddingClient(unittest.TestCase):
    def test_looks_up_function_and_returns_response(self) -> None:
        stub_fn = StubRemoteFunction(
            response={
                "embeddings": [[0.1, 0.2]],
                "timestamps": [0.0],
                "model_name": "ast",
        ...
        fake_modal = SimpleNamespace(
            Function=SimpleNamespace(
                from_name=lambda app_name, function_name: stub_fn
            )
        )
        ...
        self.assertEqual(len(stub_fn.calls), 1)
        self.assertEqual(stub_fn.calls[0]["request_id"], "req-1")
        self.assertEqual(response.embedding_dim, 2)
        self.assertEqual(response.timestamps, [0.0])
    ...
    def test_wraps_remote_errors(self) -> None:
        ...
        with patch("search.modal_embedding_client.modal", fake_modal):
            client = ModalEmbeddingClient("app", "fn", timeout_seconds=1.0)
            with self.assertRaises(ModalEmbeddingError):
                client.embed(
                    ModalEmbeddingRequest(
                        wav_bytes=b"wav",
                        request_id="req-1",
