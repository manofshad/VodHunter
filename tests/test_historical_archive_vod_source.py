...
class TestHistoricalArchiveVODSource(unittest.TestCase):
    def _make_vod(self) -> dict[str, object]:
        return {
            "id": "vod-1",
            "url": "https://www.twitch.tv/videos/vod-1",
            "title": "Historical stream",
    ...
    def test_resumes_from_saved_cursor(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            source.start()
            self.assertEqual(source.ingest_cursor_seconds, 120)

            chunk = source.next_chunk()
            self.assertIsNotNone(chunk)
            assert chunk is not None
            self.assertEqual(chunk.offset_seconds, 120.0)
            self.assertEqual(store.vod_state["vod-1"]["last_ingested_seconds"], 120)

            source.next_chunk()
            self.assertEqual(source.ingest_cursor_seconds, 180)
    ...
    def test_finalize_marks_video_processed_and_clears_state(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            source.start()
            self.assertIn("vod-1", store.vod_state)
            ...
            self.assertNotIn("vod-1", store.vod_state)
            self.assertTrue(source.is_finished)
